...
describe('connection client apply', () => {
  ...
  it('mints unary rpcIds without requiring secure-context randomUUID', async () => {
    ;(globalThis as Win).location = {
    ...
    try {
      await expect(handle.api.workspace.list({}))
        .resolves.toMatchObject({ result: { ok: true, value: { items: [], archivedSessionIds: [] } } })
    } finally {
      globalThis.fetch = original
      vi.unstubAllGlobals()
    }
    expect(seen).toHaveLength(1)
    expect(seen[0]?.url).toBe('http://192.0.2.20:3080/api/workspace.list')
    expect(seen[0]?.body).toMatchObject({
      type: 'client-request',
      rpcId: '00000000-0000-4000-8000-000000000000',
